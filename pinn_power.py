...
class PowerMethodPINN:
    ...
    def evaluate_errors_and_plot(self, x_eval, u_pred, u_true):
        ...
        # Check if the eigenfunction is inverted
        if np.max(u_pred) * np.max(u_true) < 0:
            print("Eigenfunction is inverted. Correcting...")
            u_pred = -u_pred  # Flip the sign to correct the inversion
